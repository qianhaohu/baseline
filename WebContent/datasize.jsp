...
<script>
	$(function() {
		...
		//首次显示
		url = "DataSizeServlet?dbname=finance&bizdate=day&item_name=arch_size";
		...
		$("select").change(
				function() {
					...
					url = "DataSizeServlet?dbname=" + dbname + "&bizdate="
							+ bizdate + "&item_name=" + item_name;
					//从后台获取json格式的数据  
					$.getJSON(url, function(data) {
						//初始化chart  
						var chart = new Highcharts.Chart(data);
...
<body>
	...
	<select id="item_name">
		<option value="arch_size" selected="selected">arch_size</option>
		<option value="asm_disk_size">asm_disk_size</option>
		<option value="tablespace_size">tablespace_size</option>
		<option value="data_size">data_size</option>
	</select>
	<select id="bizdate">
